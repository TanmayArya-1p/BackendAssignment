export default class ItemFilter {
  ...
  render() {
    for (const i of this.items) {
      ...
      if (
        ...
        this.filterTags.some((tag) => i.tags.map((a) => a[1]).includes(tag))
      ) {
  ...
  init() {
    ...
    for (const i of this.items) {
      ...
      item.querySelector(".item-ctr").textContent = this.ctrMap[i.id].ctr
        ? this.ctrMap[i.id].ctr
        : 0;
      item.querySelector(".item-incr").addEventListener("click", () => {
        this.ctrMap[i.id]++;
        this.addedPrice += i.price;
        this.updateAddedPrice();
        item.querySelector(".item-ctr").textContent = Number(
          this.ctrMap[i.id] ? this.ctrMap[i.id] : 0,
        );
      });
      item.querySelector(".item-decr").addEventListener("click", () => {
        if (this.ctrMap[i.id] > 0) {
          ...
          this.addedPrice -= i.price;
          this.updateAddedPrice();
          item.querySelector(".item-ctr").textContent = this.ctrMap[i.id]
            ? this.ctrMap[i.id]
            : 0;
        }
      });
      item.querySelector(".item-instructions").value =
        this.instructionMap[i.id] || "";
      item
        .querySelector(".item-instructions")
    ...
    for (const i of this.items) {
      if (this.ctrMap[i.id] > 0) {
        items.push({
          ...
          instructions: this.instructionMap[i.id]
            ? this.instructionMap[i.id]
            : "",
        });
      }
