...
export async function getAllOrderedItems() {
  let items = await db.query("SELECT * FROM order_items ORDER BY issued_at");
  return items[0];
}
...
export async function bumpStatus(id) {
  ...
  await db.query("UPDATE order_items SET status = ? WHERE id = ?", [
    bumpMap[item.status],
    id,
  ]);
...
export async function evaluateOrderStatus(id) {
  ...
  let minStatus = order.ordered_items.reduce(
    (acc, curr) => (acc < curr.status ? acc : curr.status),
    "served",
  );
